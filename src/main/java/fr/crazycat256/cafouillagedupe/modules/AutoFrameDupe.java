...
public class AutoFrameDupe extends Module {
    ...
    private final Setting<Integer> minStackSize = sgGeneral.add(new IntSetting.Builder()
        .name("min-stack-size")
        .description("The minimum stack size to use when duping (lower = faster but more inventory management).")
        .defaultValue(1)  // Lowered from 5 to 1 for more aggressive duping
        .min(1)
        .sliderMax(64)
    ...
    private final Setting<Integer> maxPlacements = sgGeneral.add(new IntSetting.Builder()
        ...
        .defaultValue(64)  // Increased from 12 to 64 for faster placement
        .min(0)
        .sliderRange(1, 128)  // Increased max slider range
        .build()
    );
    ...
    private final Setting<Integer> maxSwaps = sgGeneral.add(new IntSetting.Builder()
        ...
        .defaultValue(16)  // Increased from 8 to 16 for faster inventory management
        .min(0)
        .sliderRange(1, 32)  // Increased max slider range
        .build()
    );

    private final Setting<Integer> maxInventoryMoves = sgGeneral.add(new IntSetting.Builder()
        .name("max-inventory-moves")
        .description("The maximum number of item moves from inventory to hotbar to perform per tick (increased for better performance).")
        .defaultValue(10)  // Increased from 0 to 10 for faster inventory management
        .min(0)
        .sliderMax(40)  // Increased max slider range
        .build()
    );
